...
# ---------------- EXAMORA KNOWLEDGE BASE SEARCH ---------------- #

def search_examora_knowledge(
    board,
    class_name,
    group_name,
    question
    ...
    ignored_words = {
        ...
        "and", "or", "of", "to", "in", "on",
        "for", "with", "how", "why", "when",
        "where", "explain", "tell", "me",
    ...
    cursor.execute("""
        ...
        FROM knowledge_base
        WHERE
            (board = ? OR board IS NULL OR board = '')
            AND
            (class_name = ? OR class_name IS NULL OR class_name = '')
            AND
            (group_name = ? OR group_name IS NULL OR group_name = '')
        ORDER BY id DESC
    """, (
        board,
        class_name,
...
def ai_tutor_api():
    ...
    try:
        ...
        # -----------------------------------------
        # GET CURRENT STUDENT'S ACADEMIC CONTEXT
        # -----------------------------------------
        ...
        if student:
            board = student[0] or "Not specified"
            class_name = student[1] or "Not specified"
            group_name = student[2] or "Not specified"
        else:
            board = "Not specified"
        ...
        # -----------------------------------------
        # SEARCH EXAMORA KNOWLEDGE BASE FIRST
        # -----------------------------------------

        knowledge_results = search_examora_knowledge(
            board,
            class_name,
            group_name,
            user_message
        ...
        # Use Examora's knowledge only when there is
        # a meaningful match with the student's question.
        if knowledge_results and knowledge_results[0]["score"] >= 2:
            ...
            return jsonify({
                ...
                "provider": "Examora Knowledge Base",
                "source": best_result.get("source"),
                "subject": best_result.get("subject"),
                "chapter": best_result.get("chapter")
            })

        # -----------------------------------------
        # BUILD EXAMORA-SPECIFIC AI PROMPT
        # -----------------------------------------

        examora_prompt = f"""
You are Examora AI Tutor.

You are helping a student inside the Examora educational platform.
...
1. Answer according to the student's class level.
2. Prefer the student's board/curriculum context when relevant.
3. Explain difficult concepts in simple language.
4. Do not deliberately make the answer more advanced than necessary.
5. Give examples when they help understanding.
6. If the question is academic, focus on learning rather than simply giving an unexplained answer.
7. Never invent facts, syllabus information, formulas, or references.
8. If you are uncertain about board-specific information, clearly say that you are uncertain.
9. Do not claim that you have verified information from an official source unless Examora actually provided that source.
10. The student's question should be answered directly first.

You are an AI tutor, not an unquestionable authority.
...
"""
        ...
        # -----------------------------------------
        # ASK EXAMORA AI PROVIDER ROUTER
        # -----------------------------------------

        result = ai_router.ask_examora_ai(examora_prompt)
        ...
        return jsonify({
            "success": False,
            "error": (
            "The current model does not have an answer for this question. "
            "We will continue improving Examora, and our future models will be more helpful."
        )
        }), 503
