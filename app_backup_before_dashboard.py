...
def submit_exam(paper_id):
    ...
    # --- 1. Grading Logic (Existing) ---
    cursor.execute("SELECT mcq_answers FROM papers WHERE id=?", (paper_id,))
    ...
    cursor.execute("SELECT short_keywords FROM papers WHERE id=?", (paper_id,))
    ...
    cursor.execute("SELECT long_keywords FROM papers WHERE id=?", (paper_id,))
    ...
    if percentage <= 20: level = "critical"
    elif percentage <= 40: level = "weak"
    elif percentage <= 80: level = "average"
    else: level = "excellent"

    # --- 2. Saving Operations (Must occur while conn is open) ---
    
    # Save to 'results'
    cursor.execute(
        "INSERT INTO results(username, paper_id, mcq_score, total_mcqs, percentage) VALUES(?, ?, ?, ?, ?)",
        (session['user'], paper_id, mcq_score, total_mcqs, percentage)
    )

    # Save to 'exam_history' (New)
    cursor.execute(
        "INSERT INTO exam_history(username, subject, score_obtained, test_date) VALUES(?, ?, ?, DATETIME('now'))",
        (session['user'], "General", total_score)
    )
    ...
    # --- 4. Return the result page (No database operations here) ---
    return render_template(
        'result.html',
        score=total_score,
...
# ---------------- RESULT LEVEL SYSTEM ---------------- #
    ...
    # ================= NEW: SAVE TO EXAM_HISTORY =================
    cursor.execute(
        """
        INSERT INTO exam_history(username, subject, score_obtained, test_date)
        VALUES(?, ?, ?, DATETIME('now'))
        """,
        (session['user'], "General", total_score)
    )
...
def leaderboard_page():
    ...
    # 1. Fetch user's board for personalized matrix
    cursor.execute("SELECT board FROM users WHERE username = ?", (current_username,))
    user_record = cursor.fetchone()
    user_board = user_record['board'] if (user_record and user_record['board']) else 'Federal Board'

    # 2. Fetch National and Board level datasets
    cursor.execute("SELECT username, total_marks, board, class_name, profile_picture FROM users ORDER BY COALESCE(total_marks, 0) DESC")
    national_rows = cursor.fetchall()

    cursor.execute("SELECT username, total_marks, board, class_name, profile_picture FROM users WHERE board = ? ORDER BY COALESCE(total_marks, 0) DESC", (user_board,))
    board_rows = cursor.fetchall()
    conn.close()

    # 3. Transform to lists with sequential positions
    national_list = [{**dict(row), 'position': i+1} for i, row in enumerate(national_rows)]
    board_list = [{**dict(row), 'position': i+1} for i, row in enumerate(board_rows)]
