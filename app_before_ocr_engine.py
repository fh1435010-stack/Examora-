...
import requests
from source_normalizer import save_source_group
...
def knowledge_upload():
    ...
    if request.method == 'POST':
        ...
        if upload_type == "pdf":
            ...
            if len(files) != 1:
                return "Please upload exactly one PDF file.", 400

            if not files[0].filename.lower().endswith(".pdf"):
                return "The selected file must be a PDF.", 400
        ...
        try:
            ...
            return (
                "Knowledge source uploaded successfully. "
                f"Source ID: {result['upload_id']} | "
                f"Files: {result['file_count']}"
            )

        except Exception as error:

            print("KNOWLEDGE UPLOAD ERROR:", error)

            return "Knowledge source upload failed.", 500

    return render_template('knowledge_upload.html')
    ...
    try:
        ...
        if knowledge_results:
            ...
            return jsonify({
                "success": True,
                "answer": best_result["content"],
                "provider": "Examora Knowledge Brain",
                "source": best_result.get("source"),
                "subject": best_result.get("subject"),
                "chapter": best_result.get("chapter")
            })
        ...
        return jsonify({
            "success": False,
            "error": (
                "I could not find enough approved knowledge in the "
                "Examora Knowledge Brain to answer this question yet."
            )
        }), 404

    except Exception as error:

        print("EXAMORA TUTOR ERROR:", error)

        return jsonify({
            "success": False,
            "error": "Examora Knowledge Brain error"
        }), 500
